...
---@param path string
---@return table
function M.resolve_path(colors, path, inheriting)
  inheriting = inheriting or false
  ...
  for i, v in pairs(path_spl) do
    ...
    elseif i == #path_spl
        ...
        and node[v].dark ~= nil then
      return node[v]
    elseif i == #path_spl
        and type(node[v]) == "string"
        -- Allows only one level of inheritance
        and not inheriting then
      return M.resolve_path(colors, node[v], true)
    else
